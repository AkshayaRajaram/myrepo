		...
		.ready(
				function() {
					...
					if (status === 'true') {
						...
						$("input[name=radiogrp][value='" + cType + "']").prop(
								'checked', true);
						$("input[type=checkbox][value='" + pType + "']").prop(
								'checked', true);
						$("input.plan").prop("disabled", true);
						$("input[type=checkbox][value='" + pType + "']").prop(
								'disabled', false);
						...
						$('#underwriters')
								.empty()
								.append(
										'<option selected="selected" value="0">Please select an underwriter</option>');
						$.each(JSON.parse(u_list), function(index, value) {
							$('#underwriters').append(
									$("<option></option>").val(
											value.underWriterId).html(
													...
													.after(
															"<tr><td>"
															 +
																	...
																	+ "</td><td>"
																	+ value.name
																	+ "</td><td> <button id='but1' class='botaoadd'>initiate </button><a href='accountInfo.htm?id="
																	+ value.accountId
																	+ "'>fail</a> </td></tr>");
										});
					}
									...
									function() {
										...
										$('#underwriters')
												...
												.append(
														'<option selected="selected" value="0">Please select an underwriter</option>');
										$(".accTable").hide();
										var enabledProducts = cov_Type == "B" ? "B"
												: cov_Type == "S" ? "S"
														: cov_Type == "G" ? "G"
																: cov_Type == "P" ? "P"
																		: " ";
					...
					$('.plan')
							.on(
									'change',
									function() {
										$('.plan').not(this).prop('checked',
												false);
										$(".accTable").hide();
										var coverageType = $(
												"input[name='radiogrp']:checked")
												.val();
															...
															$('#underwriters')
																	.append(
																			$(
																					"<option></option>")
																					.val(
																							data[i].underWriterId)
																					.html(
																							data[i].underWriterName));
														}
													},
